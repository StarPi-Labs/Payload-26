...
void health_monitoring_task(void *arg) {
    ...
    while (1) {
        ...
        // Lock the pointers so they don't move while we calculate
        xSemaphoreTake(buf->lock, portMAX_DELAY);
        
        size_t h = buf->head;
        size_t t = buf->tail;
        xSemaphoreGive(buf->lock); // Release lock so writers can continue
    
        if (h == t) continue; // No data actually here
    
        // Calculate available data based on circular wrap
        size_t available = (h > t) ? (h - t) : (HM_BUFFER_SZ - t + h);
        
        // Cap the read at the chunk size
        size_t to_read = (available > BT_TX_CHUNK_SIZE) ? BT_TX_CHUNK_SIZE : available;
