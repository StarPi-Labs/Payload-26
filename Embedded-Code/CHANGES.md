...
- **DevKitC two-port split:** the console runs on **USB-Serial-JTAG** (the "USB" connector; also used for flashing/monitor), and telemetry runs on **UART0's native pins GPIO43/44 @ 115200** — hardwired to the on-board CP210x bridge (the "UART" connector). Each USB port carries exactly one stream, no external wiring, and console text can never splice into the binary frames again (that corruption bit us once; the parser's CRC check is also enabled now as a second line of defense). The ROM bootloader still chirps a few ASCII lines on GPIO43 at reset — the parser's CRC/resync discards them.
